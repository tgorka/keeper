...
lives in `src-tauri/crates/keeper/tauri.conf.json` under `plugins.updater.pubkey`, so it is baked
into every build; the matching private key signs the updater artifacts at release time.

**The private key lives in 1Password** — `op://tg/keeper-updater-signing-key` — with the password
in the item's `password` field and the public half beside it. Read it with:

```sh
op read --no-newline "op://tg/keeper-updater-signing-key/credential"
```

> **This used to say the key must exist only in GitHub secrets, and that rule broke the updater.**
> A key nobody can read is a key nobody can use: when CI stopped being able to run (see the Apple
> gate below), the only copy of the signing key sat in a write-only secret, so no release could
> carry `latest.json` and every install's Update button failed with *"Could not fetch a valid
> release JSON from the remote"*. The key was rotated on 2026-08-05 (new fingerprint
> `F73E48FF43358CDF`) precisely because the old one was unrecoverable. A secret you cannot read is
> also a secret you cannot rotate, audit, or hand to a second maintainer.
>
> Rotating the keypair invalidates every **already installed** build, because the old public key is
> compiled into it: those copies must be replaced by hand once, from the dmg. After that the channel
> works again.

| Secret | What it is |
| --- | --- |
| `TAURI_SIGNING_PRIVATE_KEY` | The minisign **private key** string. Mirrored into GitHub secrets so CI can sign if it is ever able to build; the readable copy is the 1Password item above. |
| `TAURI_SIGNING_PRIVATE_KEY_PASSWORD` | The password protecting that private key. |

**To rotate the keypair (only when it has leaked, or when the current one is unrecoverable):**

```sh
bunx tauri signer generate --ci -p "<password>" -w /tmp/keeper-updater.key
```

1. Put the printed **public key** into `plugins.updater.pubkey` in
   `src-tauri/crates/keeper/tauri.conf.json` and commit it.
2. Store the **private key** and its password in the 1Password item, and mirror both into the
...
workflow's Apple-signing gate demands three notarization secrets from a paid Developer Program this
project does not have (decision D-1). `--no-bundle` builds (CI's `check`) skip updater artifacts, so
they stay green without any key.
