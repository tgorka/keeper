...
| Destination | When | Why |
| --- | --- | --- |
| **Each sync profile's git remote** (e.g. `https://forgejo.example.org/…`, `ssh://host/…`) | Every sync tick for an enabled profile | The whole point: fetch, push and LFS transfer. One entry per configured profile. |
