...
> **This file is loaded into every agent request.** Coding agents (omp, Claude Code,
> Cursor) discover `AGENTS.md` by walking up from the working directory, so every
> line here is re-sent on every turn of every session — including the subagents
> that do most of the work. That is why it lives at the repository root rather
> than under `docs/`: for six weeks it sat at `docs/project-context.md`, where
> nothing auto-loaded it, and fleet telemetry measured agents working on keeper
> with **0 tokens of repo rules** against 15k tokens of generic skills.
>
> The cost is the discipline: a rule earns its place here only if breaking it is
> expensive and the code does not already prevent it. Everything that is merely
> useful belongs in `docs/` — `decisions.md` (D-numbered project decisions),
> `constraints-and-limitations.md`, `sync.md`, `sessions.md`, `release.md`,
> `settings-keys.md` — and an agent reads those on demand.
>
> `docs/project-context.md` is a symlink to this file: the BMAD automation globs
> `**/project-context.md`, and one file with two paths cannot drift.
