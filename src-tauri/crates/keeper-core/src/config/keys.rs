...
pub const KEYS: &[KeySpec] = &[
    ...
    KeySpec {
        key: "recording.system_audio",
        family: false,
        scope: Scope::UserGlobal,
        settable: Settable::AnyLayer,
        shape: Shape::Flag01,
        default: "1",
        summary: "Whether the next Recording Session captures system audio.",
        example: "false",
    },
    KeySpec {
        key: "recording.microphone",
        family: false,
        scope: Scope::UserGlobal,
        settable: Settable::AnyLayer,
        shape: Shape::Flag01,
        default: "1",
        summary: "Whether the next Recording Session captures the microphone; the device itself is chosen per session.",
        example: "false",
    },
    KeySpec {
    ...
    KeySpec {
        ...
        summary: "Per drive: `account:<account id>` authenticates with that organisation \
                  account's sign-in token while it is the configured account; \
                  `forge:<source id>` with that repository source's connection; absent uses \
                  the drive's own saved token.",
        example: "",
    },
    KeySpec {
    ...
    KeySpec {
        ...
        shape: Shape::Flag01,
        default: "1",
        summary: "Whether keeper checks for its own update on a cadence, installs it in the background, and then restarts itself into it once nobody is using keeper — overnight, or after hours away, and never while a recording is live. Off means the update endpoint is contacted only when somebody clicks Check for updates, and only a person restarts.",
        example: "false",
    },
];
