...
# Codesign the sidecar when a signing identity is available (mirrors what the
# release workflow does, which now always signs). This is what makes a macOS TCC
# grant survive a rebuild: an ad-hoc signature's designated requirement is a bare
# cdhash, so every rebuild looks like a different app to TCC and Screen Recording
# re-prompts forever. With no identity set the binary stays ad-hoc — CI's
# `--no-bundle` build and non-recording dev work are unaffected.
