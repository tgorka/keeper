...
| the one recurring maintenance hook | the success edge `mark_synced → prune_lfs_store` `engine.rs:3185-3197` **[corrected 2026-08-22: the scouts' `engine.rs:3052-3066` is `adopt_volume`; `fn mark_synced` is at `:3185` and the prune call at `:3188-3196`]**; supervisor `TICK_MS = 1000` `engine.rs:338`; the paced due-gates `scan_is_due` `engine.rs:1304` and `sweep_is_due` `engine.rs:1390` | ships — there is **no `.timer` unit in the repository** (`keeper-syncd.service` is `ExecStart=…keeper-syncd watch`), and `docs/sync.md:889-893` refuses timer-driven *self-update* only, for a daemon that "can be mid-push at any moment". **A "nightly" pass may therefore ride this edge OR be driven externally**: `keeper-syncd sync --once` is documented as *"the cron entry point"* (`keeper-syncd/src/commands.rs:232`) and `verify --remote` *"exits non-zero so a cron wrapper sees it"* (`docs/sync.md:325-326`). See Epic 57 / AD-135…AD-137 |
