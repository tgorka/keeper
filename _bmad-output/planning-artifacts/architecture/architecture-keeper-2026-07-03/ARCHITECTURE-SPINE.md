...
scope: 'keeper MVP — macOS Tauri 2 desktop Matrix client: multi-account, bridges, local archive, drafts/approval, incognito, undo-send — plus the iOS/iPhone phase (same core, same IPC contract, phone shell), the macOS Screen Recording phase (first-party Swift capture sidecar), and the Folder Sync phase (git-protocol folder synchronization on gitoxide, in-app and as a standalone Linux daemon)'
status: final
created: '2026-07-03'
updated: '2026-07-25'
binds: [FR-1..FR-93, NFR-1..NFR-26]
sources:
...
### Folder Sync Phase (2026-07-25) — AD-40 … AD-53

*Extends the frozen AD-1..39 for PRD §15 (FR-77–FR-93, NFR-23–NFR-26), on the authoritative `research-sync-2026-07-25.md` (capability matrix, Forgejo quirk list and stability-gate tiers adopted, not relitigated). This phase adds a **second product surface** to keeper: git-protocol folder synchronization between a local directory and a Forgejo (or any git) remote, shipped both inside the app and as a standalone Linux CLI. It is deliberately **orthogonal to Matrix**: nothing here may reach into `keeper-core`'s messaging hexagon, and nothing in the messaging path may depend on sync.*

```mermaid
graph TD
  UI["src/ — Sync settings · profile list · progress · warnings"] -- "commands" --> SHELL["keeper shell (ipc.rs)"]
  SHELL -- "Channel&lt;SyncProgressVm&gt; + polled SyncStatusVm" --> UI
  TRAY["tray.rs — sync glyph frames + status line"] --> SHELL
  SHELL --> ENGINE["keeper-sync (new crate)\nprofiles · journal · scheduler · stability gate\nprovenance · conflict policy"]
  CLI["keeper-syncd (new bin crate)\nLinux CLI + daemon"] --> ENGINE
  ENGINE -- "SyncPlatform port" --> SHELL
  ENGINE --> GIX["gix 0.86 — clone · fetch · checkout · status · index · commit · attributes"]
  ENGINE --> GITCLI["git CLI shim — push · worktree · sparse-checkout · gc"]
  ENGINE --> LFS["keeper-sync::lfs — pointer · CAS store · batch API · basic transfer"]
  ENGINE --> WATCH["notify + 4-tier stability gate"]
  GIX --> REMOTE[("Forgejo / git remote")]
  GITCLI --> REMOTE
  LFS --> REMOTE
  ENGINE --> JOURNAL[("&lt;data_dir&gt;/sync.db — durable work journal")]
```

### AD-40 — Sync lives in its own crate, beside the hexagon — never inside it
- **Binds:** FR-77, FR-92; NFR-24; crate topology
- **Prevents:** gitoxide + a filesystem watcher + an HTTP transfer queue being dragged into the iOS build; the standalone CLI inheriting matrix-sdk; the messaging hexagon coupling to an unrelated subsystem
- **Rule:** Two new Cargo workspace members: **`crates/keeper-sync`** (the engine — pure decisions, no `tauri`, and deliberately **no `keeper-core`**) and **`crates/keeper-syncd`** (`[[bin]]`, the standalone CLI, depends only on `keeper-sync`). `keeper-core` **MUST NOT** depend on `keeper-sync` — the `check:core-tauri-free` gate is joined by a `check:core-sync-free` gate asserting `cargo tree -p keeper-core` names neither `gix` nor `keeper-sync`. Because it cannot use `keeper-core::platform::Platform`, `keeper-sync` defines its **own minimal `SyncPlatform` port** (`data_dir`, `secret_get/set/delete`, `notify`, `now_ms`, `free_space`, `git_program`) which the shell implements by delegating to its existing `DesktopPlatform` and the CLI implements directly against the OS. Errors are a self-contained `SyncError` (thiserror), mapped into the existing `IpcError` envelope by extending the single `to_ipc_error` funnel (AD-21) — the shell owns the mapping, the engine owns the taxonomy.

### AD-41 — Git engine split: `gix` for everything it does, a thin audited `git` shim for the five things it cannot
- **Binds:** FR-77, FR-81, FR-82; NFR-23, NFR-24
- **Prevents:** planning around a push implementation that upstream has declined to build; a GPL license-firewall breach via libgit2; ad-hoc `git` invocations scattered through the codebase
- **Rule:** **`gix 0.86`** (MIT OR Apache-2.0) owns clone, fetch, checkout, `status`, index read/write, object and commit creation, `.gitattributes`/filter resolution, progress reporting and interruption. Push is **not implemented in gitoxide and is not planned** (issue #306 closed `NOT_PLANNED` 2026-07-22 → unreviewed discussion #2776), so **push, `worktree add|remove|prune`, `sparse-checkout init|set|reapply`, `gc`/`repack`, and — found during implementation — `merge` shell out to the `git` binary** through **one module, `keeper_sync::git_cli`** — every invocation typed, argument-vectored (never a shell string), timeout-bounded, with stdout/stderr captured and mapped to `SyncError`. **`git2`/`libgit2-sys` is banned**: it declares `MIT OR Apache-2.0` in crate metadata while vendoring GPL-2.0-with-linking-exception C, so `cargo deny` would pass it silently — the ban is recorded in `deny.toml` `[bans].deny` so the firewall is mechanical, not a matter of memory. Applying fetched commits is the fifth: gitoxide wires up no `merge`/`reset`/`restore`/`switch` workflow at all, and a fetch alone only moves the remote-tracking ref — without an apply step the working tree silently stays behind and the next push is rejected as non-fast-forward (found by the first end-to-end run, not by reading the docs). A `git` binary (and, for `file://`/`ssh://` transports, its `git-upload-pack`/`ssh` helpers) is therefore a **declared hard runtime prerequisite**: probed at startup via `SyncPlatform::git_program`, surfaced as `CapabilitiesVm.sync` and, when missing, as a loud actionable warning rather than a silent degradation.

### AD-42 — A sync profile is the unit of everything; the journal is the truth
- **Binds:** FR-78, FR-90; NFR-24
- **Prevents:** a half-applied sync being lost to a crash or a quit; two profiles fighting over one working tree; settings sprawl
- **Rule:** A **`SyncProfile`** binds one local folder to one remote repo and is the unit of configuration, concurrency, progress, logging and failure: `{id (ULID), name, local_path, remote_url, branch, direction, subpaths[], removable, lfs_policy, schedule, exclude[], identity}`. Profiles are persisted in a **third SQLite database `<data_dir>/sync.db`** (WAL, opened and migrated exactly like `keeper.db`, AD-25) holding `profiles`, a durable **work journal** (`journal(id, profile_id, kind, payload, state, attempts, not_before_ms, created_ms)`), and a `file_state` cache keyed by `(profile_id, path)` for the stability gate. Writes go through **one serialized writer task per database** (the `ArchiveWriter::spawn` precedent) so no `Connection` is ever held across an `.await`. Every unit of work is journaled **before** it is attempted and cleared only after it is durably observable in the remote or the worktree — an interrupted push, LFS upload or checkout is re-driven on the next tick, never lost. At most **one active operation per profile** (an RAII reservation, the `LiveFolderReservation` precedent); profiles are otherwise fully concurrent. Human-editable overrides ride the existing `config.json` boot-time import (AD-25) under a `sync.*` key namespace.

### AD-43 — Convergence without prompts: conflict copies, never a modal
- **Binds:** FR-77, FR-89; NFR-24
- **Prevents:** a sync that stalls forever waiting for a human; silent loss of one side of a divergence; merge-conflict markers landing in a user's binary files
- **Rule:** The engine **never blocks on a user decision to converge.** Local change → stability gate → stage → commit → push. Remote change → fetch → fast-forward when possible. On divergence the engine **rebases nothing and merges nothing by content**: it adopts the **Syncthing resolution shape** — the remote revision wins the canonical path, and the local revision is preserved verbatim as **`<stem>.sync-conflict-<UTC yyyymmdd-hhmmss>-<device>.<ext>`**, committed as an ordinary tracked file so every peer sees the same pair. Deletions never beat modifications (a modified file resurrects over a remote delete, and the delete is re-applied only if the file is untouched). Text files get no three-way merge in v1 — a conflict copy is honest and reversible where an auto-merge is neither. Every conflict raises a **non-blocking warning** (AD-51) naming both paths. The only conditions permitted to *require* a human are the ones no policy can decide: an invalid path for the remote filesystem (rename required), an exhausted quota, and a credential that is rejected — each surfaced as an actionable notice, never a blocking modal.

### AD-44 — Provenance is a commit trailer, and the device is a first-class identity
- **Binds:** FR-86; NFR-26
- **Prevents:** an unattributable change in a shared repo; provenance stored in a sidecar file that drifts from history; a bot commit indistinguishable from a human one
- **Rule:** Provenance rides **git's own metadata**, never a parallel file. Every engine-authored commit carries a fixed trailer block — `Keeper-Profile: <name>`, `Keeper-Device: <device-label> (<device-id>)`, `Keeper-Origin: <hostname-or-volume-label>`, `Keeper-Source: watch|manual|cli|bot`, `Keeper-Agent: keeper-sync/<version>` — plus any per-profile `Keeper-Tag:` lines the user configured. The **device identity** is a ULID minted once per installation, stored in `sync.db`, paired with a human label; the git author is `"<user or device label> <sync@<device-id>.keeper.invalid>"` unless the profile overrides it, so history is attributable without leaking a real address by default. Commit subjects are generated and stable (`sync(<profile>): 3 added, 1 modified, 1 deleted`) with the file list in the body; a bot lane (AD-50) substitutes its own subject. Trailers are parsed back out for the UI, so "where did this change come from" is answerable offline from the clone alone.

### AD-45 — Only complete, verified files enter a commit — the four-tier gate
- **Binds:** FR-84, FR-85; NFR-24, NFR-25
- **Prevents:** a half-downloaded browser file being committed and pushed; a torn read producing a corrupt blob; an iCloud placeholder being silently materialized
- **Rule:** A changed path becomes eligible for staging only after passing four tiers, **no one of which may decide alone**: **(0) name/shape exclusion** — a `globset` seeded from the industry list (`*.crdownload`, `*.part`, `*.filepart`, `~$*`, `.~lock.*#`, `.*.sw?`, `.DS_Store`, `._*`, `.fuse_hidden*`, `.nfs*`, the `**/*.download/**` Safari *package directory*, our own `.keeper.*.tmp` staging prefix) plus per-profile excludes; **(1) an event trigger** — `notify` + `notify-debouncer-full`, where Linux `IN_CLOSE_WRITE` is a *fast path* that shortens the window but is never a proof (close-then-reopen is ordinary), and macOS has **no close event at all** in any available API; **(2) a quiescence window** — `(size, mtime_ns, ctime_ns, ino)` byte-identical across two samples ≥ *W* apart **and** `now − mtime ≥ W`, with `W` = 1 s after a Linux close-write, **5 s default**, 10 s on removable/network media, a **60 s ceiling** that forces an attempt regardless, and Nextcloud's future-mtime escape hatch (`mtime > now + 10 s` never holds a file hostage to a broken clock); **(3) an open-writer veto** — Linux-only and best-effort: `/proc/locks` (free, always on) then optionally `/proc/*/fd` + `fdinfo` access-mode for large candidates; **macOS has no tier 3** and must say so in the docs (`lsof` is too slow, `libproc` needs root, EndpointSecurity needs an Apple-granted entitlement); **(4) verify-on-read** — the only actual proof: `fstat` the *open fd* before and after, stream-hash with `sha2`, and treat any change as a **soft, retryable** requeue, never a user-facing failure. On macOS every candidate is `lstat`ed for **`SF_DATALESS` (0x40000000)** and skipped if set — opening a dataless file silently materializes it and would drag a user's entire iCloud Drive through the sync engine.

### AD-46 — Large content is LFS content, and the LFS client is ours
- **Binds:** FR-80, FR-85; NFR-23
- **Prevents:** a multi-GB video becoming a multi-GB `Vec<u8>`; unbounded repository growth; a hard dependency on a `git-lfs` binary that may not be installed
- **Rule:** gitoxide has **no streaming object read** — `try_find` always fills a buffer, so a 3 GB blob is a 3 GB allocation — and `gix-lfs` is an **empty `0.0.0` placeholder**. Therefore **LFS is mandatory, not optional**: any file above a per-profile threshold (default **4 MiB**) is tracked through `.gitattributes` (`filter=lfs diff=lfs merge=lfs -text`), which the engine **maintains automatically** and commits with provenance, and the raw bytes never enter the ODB. The LFS client is **first-party, in `keeper_sync::lfs`** (`pointer`, `store`, `endpoint`, `batch`, `basic`, `queue`), built on our existing `reqwest 0.13` + `sha2` + `tempfile` — the `rustutils/git-lfs` crate family is a **design template only** (single author, published 2026-05-13, ~300–600 downloads, pinned to `reqwest 0.12`, which would duplicate the whole hyper/rustls stack). Filtering is **in-process**: the engine resolves `.gitattributes` itself via `gix-attributes` and substitutes pointer↔object in its own checkout/commit loop rather than registering an external `filter.lfs.process` driver — deterministic, subprocess-free, and it lets a profile choose *pointer-only* versus *materialize* per subpath (the `fetchinclude`/`fetchexclude` idiom, which is the only lever that reduces LFS traffic since git-lfs is sparse-checkout-unaware). Transfers stream and hash in one pass, verify the SHA-256 OID **and** the byte count, resume downloads by `Range` (capped below 2^31 for Forgejo's 32-bit parser, validating only the `Content-Range` *start* byte because Forgejo's complete-length is wrong), and accept that `basic` **uploads are not resumable** — they retry from zero, and `tus`/`multipart` are not implemented anywhere worth targeting.

### AD-47 — Partial syncs are sparse checkouts, with the sparse index disabled
- **Binds:** FR-81; NFR-23
- **Prevents:** `gix::status` hard-failing on a repository git itself created; a "partial" profile that still downloads everything
- **Rule:** A profile that syncs a subset of a repo declares `subpaths[]`, applied with **`git sparse-checkout set --cone`** through the `git_cli` shim, and **`index.sparse=false` is forced** in every managed repository's config at clone time (`with_in_memory_config_overrides(["index.sparse=false"])`) and written to `.git/config`. This is not a preference: `gix::status` **hard-fails** with `TreeIndexDiff(IsSparse)` on a true sparse index, while a non-sparse index carrying `SKIP_WORKTREE` flags is fully understood by gix (verified: status reports zero changes and the flags survive a gix index round-trip). The engine additionally applies the same `subpaths[]` as its **own LFS path filter**, because sparse checkout does not reduce LFS traffic. A profile may also be **shallow** (`Shallow::DepthAtRemote`) and is by default for first clone of a large history; deepening is an explicit, journaled operation.

### AD-48 — Removable media: absence is not deletion
- **Binds:** FR-83; NFR-24
- **Prevents:** the catastrophic failure mode of this feature — an unplugged pendrive being interpreted as "the user deleted 40 GB" and propagated to the server
- **Rule:** A profile marked `removable` is bound to the **volume, not the path**: identity is a `.keeper-sync/volume.json` marker (volume ULID, label, profile ids, schema version) written at the mount root and matched on every scan, so a re-mount at a different path is still the same volume. When the marker is absent the profile enters **`Paused (media absent)`** — a first-class state, not an error: the watcher is torn down, the journal is retained intact, **no deletion is ever staged, committed or pushed**, and the tray/UI says exactly why. On re-attachment the profile resumes from the journal and reconciles. Deletions originating from a removable profile are only honored when the volume is present *and* the file is genuinely absent from a mounted, marker-verified tree. Repositories on removable media are opened with **`gix::open::Options::default().with(Trust::Full)`** after the marker verifies the media is ours, because gitoxide's default `Trust::Reduced` on a directory not owned by the current user **silently drops all repo-local `filter.*` config** — which would mean the LFS clean filter never runs and a multi-GB file is committed raw, with no error. The engine also guards the mirror hazard: unmount detected mid-transfer aborts the operation as retryable rather than truncating.

### AD-49 — Offline is the normal case; work is scheduled, not attempted
- **Binds:** FR-90; NFR-24, NFR-25
- **Prevents:** a burst of failing network calls while a laptop is on a train; a sync that never recovers after the network returns; local git being blocked on connectivity
- **Rule:** Local git operations — staging, committing, worktree reads, status — **never require the network** and are never gated on it. Only fetch, push and LFS transfers are; those are journaled units driven by a **per-profile supervisor** modeled on `run_reconnect_supervisor` (AD-19): a `tokio::time::interval` tick with `MissedTickBehavior::Delay` re-reads the journal, dispatches every unit whose `not_before_ms` has elapsed (including those that elapsed while the app was down), and applies **exponential backoff with jitter** — the first generic backoff utility in the repo, so it lives in `keeper_sync::backoff` and is unit-tested against a paused tokio clock. Connectivity is *observed, not predicted*: a failed transfer classified as a network fault schedules a retry and flips the profile to `Offline`, and the next success flips it back — there is no reachability poll and no captive-portal heuristic. Suspend/resume reuses `should_restart_sync`'s wall-clock `SystemTime` gate (AD-30) rather than `Instant`, which does not advance across sleep. Quit-while-syncing follows the recording precedent: bounded finalize, then abort with the journal intact — an aborted push is always resumable, never half-applied.

### AD-50 — One-way worktree lanes are the bot-to-human airlock
- **Binds:** FR-82; the Drafts/Approval posture of AD-15
- **Prevents:** an autonomous agent writing directly into a human's synced folder; a bot's work-in-progress being force-pushed over review feedback
- **Rule:** A profile with `direction = push_only` and `lane = worktree` is the **bot lane**: the engine materializes a linked worktree (`git worktree add`, via the shim — gitoxide's worktree API is read-only) on a generated branch `keeper/<profile>/<ulid>`, the agent writes only there, and the engine commits with `Keeper-Source: bot` provenance and pushes that branch — **never the base branch, never a force-push**. The handoff to a human is a **pull request** opened against the Forgejo API, whose number and URL are recorded in the journal and surfaced in the UI; from that point the change is the human's to accept. Pull-only and bidirectional profiles never create lanes. The lane's worktree is pruned after the PR merges or the profile is removed, and a lane whose branch has diverged remotely stops and warns rather than resolving — this is the one place where a human decision is the *point*, not a failure.

### AD-51 — Sync surfaces: a capability flag, glyph frames, one status line, and warnings that never block
- **Binds:** FR-87, FR-88; NFR-25
- **Prevents:** dead sync UI on a machine with no `git`; an invisible multi-gigabyte transfer; a toast for a persistent condition
- **Rule:** `CapabilitiesVm` gains **`sync: bool`** (AD-27), true only on desktop with a usable `git` binary — every sync surface renders off that flag, never off platform sniffing. The tray (AD-39, `tray.rs`) gains a **sync glyph set**: monochrome+alpha template PNGs distinguished by **shape, not colour**, comprising an idle-with-sync-armed glyph, **four rotating arc frames** advanced by the existing ~1 Hz tick to signal activity, a paused/media-absent glyph, and a warning glyph — composed with the recording state by a single pure `decide_tray_state` so two subsystems can never fight over the icon. Progress is text in the held `status_item` (`MenuItem::set_text`, no menu rebuild, no flicker): `"Syncing tgdrive — 42/310 files · 1.2 GB of 4.7 GB"`. In-app, sync streams a `Channel<SyncProgressVm>` (the `export_start` precedent) and exposes a polled `SyncStatusVm` snapshot so the tray renders with no webview subscribed. Warnings follow the loud-failure doctrine (AD-39/Story 18.4): a sticky `warning` on the VM, an amber non-dismissible `role="alert"` banner, and **exactly one** native notification per `None→Some` onset; decision-requiring notices get an inline action button (the `ConversationHealthBanner` shape). **No toasts for connectivity or any persistent condition — ever.** All sync activity is logged through `tracing` into the existing gated `debug_log` sink, which gains an **XDG branch** (`$XDG_STATE_HOME/keeper/keeper.log`) since `app_log_path()` is macOS-only today and the CLI runs on Linux; logs carry ids, paths and byte counts, never credentials or file content.

### AD-52 — `keeper-syncd`: the same engine, no app required
- **Binds:** FR-92; NFR-24
- **Prevents:** a second, divergent sync implementation on the server; a CLI that can only be driven by a GUI's database
- **Rule:** **`keeper-syncd`** is a Linux-first `[[bin]]` crate over the *same* `keeper-sync` engine — no forked logic, no reimplemented policy. It supplies its own `SyncPlatform` implementation (XDG paths: `$XDG_CONFIG_HOME/keeper-sync/config.toml`, `$XDG_DATA_HOME/keeper-sync/sync.db`, `$XDG_STATE_HOME/keeper-sync/`; secrets from the OS keyring or, headless, from a `0600` file or environment variable, never from a config committed to git). Subcommands are the engine's verbs: `add`, `list`, `status`, `sync [--once]`, `watch`, `pause`, `resume`, `verify`, `doctor`, `logs`. It ships a **user systemd unit** (`keeper-syncd.service`, `Type=notify`-less simple, `Restart=on-failure`) and honours `SIGTERM` as a bounded graceful finalize identical to the app's quit path. Config is TOML mapping 1:1 onto `SyncProfile`, so a profile can be moved between the app and the daemon by copying a table. The CLI is **not** packaged inside the macOS app bundle; it is built and shipped separately (`cargo build -p keeper-syncd --release`) and its presence is never assumed by the app.

### AD-53 — Egress honesty and credential posture extend to git remotes
- **Binds:** FR-93; NFR-26; AD-22 posture
- **Prevents:** the app's verifiable egress claim silently becoming false; a token in a config file, a log line, or a commit
- **Rule:** Every configured sync remote is a **disclosed egress destination**: `EgressKind` gains a `GitRemote` variant and `compute_egress` a branch, so Settings → About enumerates each profile's remote host exactly as it enumerates homeservers and the updater — the disclosure is computed from the live profile set, never hand-maintained. Credentials never touch `sync.db` or `config.json`: they live behind `SyncPlatform::secret_*` (macOS Keychain / OS keyring in the app, keyring-or-`0600`-file in the daemon), are injected into gitoxide through the **programmatic `Connection::set_credentials` callback** (no helper subprocess), and into the LFS client as an `Authorization` header built per request. SSH remotes delegate key handling entirely to the external `ssh` binary and the user's own agent/config — keeper never reads a private key. Logs and error messages carry ids, hosts, paths and byte counts and are asserted secret-free by the same taxonomy rule as AD-21; the pre-commit secret scan is extended to the new crates.

*Reliability & scale envelope (PRD §15.4 — authored bars, owner sign-off at phase release, mirroring the AD-22/NFR-3 posture): **NFR-23** a 100 000-file / 50 GB profile scans and reaches steady state without unbounded memory, with per-file overhead dominated by one `lstat` (~1.2 µs measured) and LFS objects streamed, never buffered; **NFR-24** no data loss under kill -9, unplug or network loss at any point — every unit is journaled before attempt and re-driven after, and an interrupted LFS download resumes by `Range`; **NFR-25** a settled local change is committed within the profile's quiescence window + one tick, and no sync operation blocks the UI thread or the Matrix sync loop; **NFR-26** no credential or file content in logs, config, commits or error messages, and no `unsafe` in either new crate. Ownership split: the engine owns policy, journaling and verification; the shell owns timeouts, free-space probes, tray rendering and the quit bound; the `git` binary owns exactly the five operations named in AD-41 and nothing else.*
...
| `gix` (gitoxide) | 0.86.x, MSRV 1.85, MIT OR Apache-2.0; `default-features = false` with `sha1` (mandatory — omitting a hash feature fails the build), `blocking-http-transport-reqwest-rust-tls` (unifies on our `reqwest 0.13`), `credentials`, `attributes`, `worktree-mutation`, `status`, `index`, `dirwalk`, `excludes`, `revision`, `blob-diff`, `tree-editor`, `parallel`, `max-performance`, `progress-tree`, `tracing` (AD-41) |
| `git` binary (runtime prerequisite) | ≥ 2.42 for `git sparse-checkout --cone` + `ls-files --format`; used **only** for push, worktree mutation, sparse patterns and gc (AD-41). `git2`/`libgit2-sys` is **banned** in `deny.toml` `[bans].deny` (declares MIT/Apache, vendors GPL-2.0-with-linking-exception C) |
| `notify` / `notify-debouncer-full` | 8.x (CC0-1.0, allow-listed) / 0.7.x (MIT OR Apache-2.0) — inotify on Linux (incl. `IN_CLOSE_WRITE`), FSEvents on macOS, `PollWatcher` fallback for network/FUSE mounts (AD-45) |
| `globset` / `sha2` / `hex` / `tempfile` | 0.4.x (MIT arm of `Unlicense OR MIT`) / 0.10.x (hardware SHA-NI + ARMv8-SHA2 at runtime, **no `asm` feature**) / 0.4.x / 3.x — all already resolved transitively in `Cargo.lock`, promoted to direct deps of `keeper-sync` (AD-45, AD-46) |
| `keeper-sync` / `keeper-syncd` | first-party Apache-2.0 workspace members; `keeper-sync` is `tauri`-free **and** `keeper-core`-free, `keeper-syncd` is a Linux-first `[[bin]]` over the same engine (AD-40, AD-52) |
