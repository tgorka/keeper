...
### AD-41 — Git engine split: `gix` for everything it does, a thin audited `git` shim for the four things it cannot
...
- **Rule:** **`gix 0.86`** (MIT OR Apache-2.0) owns clone, fetch, checkout, `status`, index read/write, object and commit creation, `.gitattributes`/filter resolution, progress reporting and interruption. Push is **not implemented in gitoxide and is not planned** (issue #306 closed `NOT_PLANNED` 2026-07-22 → unreviewed discussion #2776), so **push, `worktree add|remove|prune`, `sparse-checkout init|set|reapply`, and `gc`/`repack` shell out to the `git` binary** through **one module, `keeper_sync::git_cli`** — every invocation typed, argument-vectored (never a shell string), timeout-bounded, with stdout/stderr captured and mapped to `SyncError`. **`git2`/`libgit2-sys` is banned**: it declares `MIT OR Apache-2.0` in crate metadata while vendoring GPL-2.0-with-linking-exception C, so `cargo deny` would pass it silently — the ban is recorded in `deny.toml` `[bans].deny` so the firewall is mechanical, not a matter of memory. A `git` binary (and, for `file://`/`ssh://` transports, its `git-upload-pack`/`ssh` helpers) is therefore a **declared hard runtime prerequisite**: probed at startup via `SyncPlatform::git_program`, surfaced as `CapabilitiesVm.sync` and, when missing, as a loud actionable warning rather than a silent degradation.
