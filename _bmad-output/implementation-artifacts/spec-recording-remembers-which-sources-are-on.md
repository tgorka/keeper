...
- `bun run check:rust:macos` (hesperia) -- NOT run; see the residual risk below.
...
**Residual risk.** `crates/keeper/src/{ipc,lib}.rs` cannot be compiled on this Linux host (AGENTS.md): the new command pair, `write_capture_sources`, the `camera_on && !audio_only` line and the two new `ipc.rs` tests were written by inspection and are gated by CI's macOS `Rust (fmt, clippy, test)` job. `cargo fmt --check` passes over them, which proves they parse. A follow-up review is recommended: the review pass moved the whole persistence path onto a new command pair after the spec was frozen, and the audio-only camera suppression changes what an existing sub-mode records.
