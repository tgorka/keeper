...
**Commands (all run, all green at `752d8e6`):**
- `bun run lint && bun run typecheck && bun run test` -- 348 files, 5857 tests.
- `cargo nextest run --manifest-path src-tauri/Cargo.toml -p keeper-core -p keeper-sync` -- 4490 tests, incl. the registry round-trip and the `config::keys` classification/doc guards.
- `cargo clippy -p keeper-core -p keeper-sync --all-targets -- -D warnings`, `cargo fmt --all --check`.
- ts-rs bindings regenerated by the keeper-core test run (`RecordingCaptureSourcesVm.ts`, `RecordingCaptureSourcesPatchVm.ts`, `RecordingSettingsVm.ts`).
- The shell-crate half, gated on CI instead (PR #359, run 34739553086): `Rust (fmt, clippy, test)` on `macos-latest` **pass** (11m15s) — so the command pair, `write_capture_sources`, the `camera_on && !audio_only` line and the two new `ipc.rs` tests compile and their tests run — plus `Tauri build (macOS)` pass, `iOS (compile check)` pass, `Frontend` pass, `License firewall` pass, `Recording sidecar` pass.

**Mutation-checked** (guard removed, suite red, guard restored): the seeding
comparison against the value each store held when the read was issued; the
per-source marking (a toggle marking all three); the patch (a write carrying all
three switches); `captureSourcesForStart` guessing a default instead of leaving
an unanswered source to Rust; the effective-answer echo; the audio-only camera
sentence; and, on the Rust side, `capture_flag`'s explicit `"0"` arm.

**Manual checks:**
- Not performed. The dev harness now round-trips the capture answer through
  `sessionStorage`, so "turn one off, reload, it is still off" is reproducible
  there, but this run did not drive a browser.

## Auto Run Result

Status: done. Blocking condition: none.

**Implemented.** Which sources a Recording Session captures is remembered. All three default ON, live in `recording.system_audio` / `recording.microphone` / `recording.camera`, are read once per launch and seeded into the session stores the Start click samples, and every toggle writes its own key back and takes the effective answer.

**Residual risk.** `crates/keeper/src/{ipc,lib}.rs` cannot be compiled on this Linux host (AGENTS.md), so its half was written by inspection — and then gated on CI, where the macOS `Rust (fmt, clippy, test)` job passed on PR #359 along with `Tauri build (macOS)`. What remains unproven is behavioural, not structural: nothing has RUN the app. Nobody has watched a switch survive a relaunch, nor started an audio-only session to see that no `camera-####.mov` appears beside the audio. A follow-up review is recommended on two counts: the review pass moved the whole persistence path onto a new command pair after the spec was frozen, and the audio-only camera suppression changes what an existing sub-mode records.
