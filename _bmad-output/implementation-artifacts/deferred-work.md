...
evidence: `a_working_clean_filter_settles_the_racily_clean_case_before_the_guard_does` in keeper-sync/tests/lfs_roundtrip.rs runs one fixture twice — with a real hashing clean filter and without — and asserts the racily-clean path reads modified iff unfiltered, while a same-length edit reads modified either way. The engine-side test can only reach the guard's `false` answer, and says so in its own doc comment, because reaching `true` needs the racily-clean re-read that goes through `filter.lfs.clean`, which under `cargo test` is the libtest harness.
status: open
