...
### DW-113: Every published macOS release is silently unsigned, so installing one is Gatekeeper-rejected AND loses the signed-in accounts — a keychain item written by a signed build is unreadable to the ad-hoc build that replaces it.
...
reason: The workflow treats Apple signing as optional and falls back to an ad-hoc build with only a `::notice::`, which nobody reads on a green run. The repository now holds only `TAURI_SIGNING_PRIVATE_KEY`/`_PASSWORD` — every `APPLE_*` secret is absent — so v0.4.0/v0.4.1/v0.4.2 all published "(unsigned build)" artifacts while the release looked entirely successful. Two consequences, the second severe and non-obvious. (1) `spctl -a` rejects the installed app, so a fresh install needs the right-click→Open ceremony. (2) macOS binds keychain ACLs to the signing identity, and an ad-hoc signature has no certificate — its identity is the cdhash, which changes with EVERY build. Installing v0.4.2 over a signed 0.3.0 on a live machine (hesperia, 2026-07-26) presented an empty login screen with both accounts gone; `keeper.db`, the SDK stores and all 8 keychain items were intact, and rolling back to the signed bundle restored both sessions immediately (20 SDK store files touched, sync connections to both homeservers re-established). Because the whole shell — including the recording surface — sits behind `App.tsx`'s signed-out `<LoginScreen/>` branch, the app also appears to have lost unrelated features. In the ad-hoc steady state this repeats on every single update. Fix: provision the `APPLE_*` secrets so the signed path runs, and make the unsigned fallback loud (fail a tagged release by default, opt in explicitly for a local/test build) rather than a notice on an otherwise-green run. Note the identity must stay stable across releases — moving between certificates costs one forced re-login, so it is worth choosing the distribution identity once and keeping it.
status: open
