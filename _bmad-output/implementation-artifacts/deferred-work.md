...
- source_spec: `{project-root}/_bmad-output/implementation-artifacts/spec-57-5-the-app-runs-them-too.md`
  ...
  status: done 2026-08-30
  resolution: |
    **Decided in favour of (a), the probe, and fixed** in story 57.5's review pass 3 —
    which is the pass that could decide it, because story 57.7's packaging had landed by
    then and the choice could be made against the unit that actually ships
    (`keeper-syncd-tasks@.timer`: a systemd **user** timer, `OnCalendar=daily`,
    `Persistent=true`, no `[Install]` — precisely the thing lingering governs).

    `HOST_SENTENCE_DAEMON` keeps "logged in or not" verbatim and becomes TRUE, because
    `daemon_presence` no longer reaches it until the caller has established lingering. A
    fourth `DaemonPresence` state, `RunsUntilLogout`, carries enabled-but-not-lingering
    with its own sentence — "the keeper-syncd unit on this machine runs this while you
    are logged in — lingering is off, so its schedule stops when your session ends".
    Same `TaskHostKind::Daemon` with two sentences, the shape
    `HOST_SENTENCE_APP_OTHER_DATA_DIR` already established under `TaskHostKind::App`, so
    the entry's predicted "new branch in the pane's host switch" did NOT materialise: no
    new host kind, no `HOST_KIND_LABELS` entry, no `.tsx` change at all. The predicted
    binding change did: `src/lib/ipc/gen/DaemonPresence.ts` gained `"runsUntilLogout"`,
    regenerated by `cargo test -p keeper-core`'s export test on Linux, never hand-edited.

    (b) was rejected on evidence rather than taste. It is the right answer only if the
    fact cannot be established cheaply and reliably, and the fact can: the entry's own
    option (a) proposed `loginctl show-user "$USER" --property=Linger`, and that
    property's getter in logind (`src/login/logind-user-dbus.c:172-190`) is one call to
    `user_check_linger_file`, whose entire body is
    `access("/var/lib/systemd/linger/<cescape(name)>", F_OK)`
    (`src/login/logind-user.c:717-737`). So `Path::exists` on that file is not a proxy
    for the subprocess — it is the same predicate, minus the fork/exec and the D-Bus
    round trip. `loginctl enable-linger` creates the file (`touch`, 0644, in a directory
    created 0755 — `src/login/logind-dbus.c:1655-1671`) and `disable-linger` unlinks it,
    so it is the state of record rather than a cache; logind also enumerates the
    directory at startup (`src/login/logind.c:288-330`). Both modes are world-readable,
    so a process asking about its own user needs no privilege, and the literal path is
    hardcoded in all three places, unchanged from systemd v219 (2015) to `main`. Cost:
    one `stat`, no subprocess, no new dependency. Taking (b) would have bought honesty by
    deleting the fact a person setting a nightly sweep needs, and weakened AD-137's
    "every host claim on screen is true" into "every host claim on screen is vague".

    The pure/impure split is the one this story's finding 3 already set with
    `profile_unreadable`: `keeper-core/src/tasks.rs` does no I/O — it owns `LINGER_DIR`
    and the pure `linger_marker_path`, which refuses `""`, `.`, `..`, any `/` and any NUL
    (`join("")` and `join(".")` both yield the directory itself, which exists on every
    systemd box, so an empty `$USER` would otherwise report lingering for a user who does
    not linger) — and `keeper/src/sync_ipc.rs`'s `daemon_lingering` does the `exists()`,
    reading `$USER` then `$LOGNAME` and answering `false` when neither is set. `false` is
    the under-claiming direction throughout. On a machine with no systemd at all the
    answer is `false` and not an error, and unreachable anyway: `systemctl --user
    is-enabled` fails there, so `Absent` is returned first.

    Tests assert the REAL strings, one per branch, so a reword cannot restore the
    over-claim silently: `a_linux_box_whose_enabled_unit_shares_the_data_dir_reads_daemon`,
    `a_linux_box_whose_enabled_unit_does_not_linger_says_the_schedule_stops_at_logout`,
    `a_host_with_no_unit_reads_app_whatever_the_lingering_fact_says`,
    `the_two_daemon_sentences_differ_and_only_the_lingering_one_claims_post_logout`, the
    `linger_marker_path` refusal pair, and the no-systemd probe. `src/test/
    task-host-tick.test.ts` gates the shell half that cannot be compiled on Linux. Both
    mutations (probe always lingering; the `RunsUntilLogout` arm rendering
    `HOST_SENTENCE_DAEMON`) fail a test.

    Everything that repeated the claim moved with it: the spec's I/O matrix (one daemon
    row became two), AD-137 in `ARCHITECTURE-SCHEDULED-TASKS.md`, D-3 in
    `docs/decisions.md`, `docs/sync.md` §14 — whose "the verdict does **not** check
    lingering" paragraph was false the moment this landed — and both unit headers, where
    `keeper-syncd-tasks@.service`'s "the app's Tasks view does NOT check lingering" is
    corrected and narrowed to the blind spot that genuinely remains (the timer itself,
    which registers nothing with keeper), and `keeper-syncd.service`'s weak "To keep
    syncing after logout" aside is promoted to the required step with its verification.
...
- source_spec: `_bmad-output/implementation-artifacts/spec-72-7-task-row-carries-every-kind.md`
  ...
  evidence: `Engine::tick` awaits `run_due_tasks(&profiles)` before it fans the per-folder passes out into its `JoinSet` (`engine.rs:3584-3589`), and `perform_copy_task` runs `copy_verified` through `Engine::blocking` (`block_in_place`, `engine.rs:2450-2457`). The tokio claim in 72.7's Design Notes is true — the runtime hands the worker's queue to a replacement thread, so nothing else on the runtime is blocked — but the *scheduler's own* pre-pass is serial, and unlike `Sync`/`Release`/`Gc`, which are bounded by a folder the task already reserves, a copy walks an arbitrary external tree of unbounded size. Left as is because moving the copy arm off the serial pre-pass is an engine-scheduling change (AD-233's territory), not a story about a task kind; the narrow claim was corrected in the spec instead of being left to read as non-starvation.
  status: open
...
### DW-256: A clone that stores media as LFS pointers salvages another machine's recording, overwriting the manifest with the pointers' lengths — and could delete the pointers outright.

origin: field measurement on hesperia, 2026-09-17, after the owner asked for `tgdrive`'s conflict copies to be resolved
location: `keeper-core/src/recording.rs` — `salvage_session_folder` (the `status:"recording"` arm), `reconcile_from_dir` (`bytes: metadata.len()`), `resolve_partial_segments` (the discard arm)
reason: the session `40-media/recordings/2026/2026-09-15 14.29 mounica-sync` carried `manifest.json` plus two `*.sync-conflict-*` copies, and the live file was the least truthful of the three: `status:"recovered"`, no `endedAt`, and its four segments recorded as 134, 135, 134 and 135 bytes — LFS pointer lengths — for captures of 837_784_110, 2_007_949_579, 889_657_113 and 1_787_690_532 bytes that were intact on disk. The older conflict copy was the real one (`finalized`, `endedAt 15:24:53`, PTS bounds, byte counts equal to the files). Mechanism: the folder reached `tgdrive-light`, which stores media as pointers, while still `status:"recording"`; that clone's `recover_orphaned_sessions` saw an inactive `recording` session, ran `reconcile_from_dir` — whose documented contract is "disk is authoritative" — over four pointer stubs, wrote `recovered` with the stubs' lengths, and committed it. Two machines writing one file is the divergence; AD-43 preserved the loser as a conflict copy, correctly. A latent worse case sits beside it: `partial_segment_is_usable` scans ISO-BMFF boxes, a pointer has no `moov`, so a `.partial` arriving as a pointer would have been **deleted** as unusable and the deletion committed — destroying that clone's only reference to media held elsewhere.
status: fixed
